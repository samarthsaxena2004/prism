...
from pipeline import _sage_stream, client
...
async def run_research_pipeline(image_b64: str, doc_id: str, facility_name: str, form_type: str = "general"):
    ...
    sage_result = None
    sage_timing = {}
    async for event in _sage_stream(image_b64, prompts.get("SAGE", "Extract all key information from this document into a structured JSON.")):
    ...
    (researcher_result, researcher_timing), (navigator_result, navigator_timing) = await asyncio.gather(
        run_researcher(client, sage_result or {}, research_prompt),
    ...
    async for event in _publisher_stream(sage_result, researcher_result, navigator_result, pub_prompt):
        if isinstance(event, dict) and event.get("final"):
